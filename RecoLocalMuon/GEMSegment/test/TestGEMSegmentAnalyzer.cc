...
class TestGEMSegmentAnalyzer : public edm::EDAnalyzer {
  ...
  std::unique_ptr<TH1F> GDTheta_GE11, GDTheta_GE21,GPTheta_GE11, GPTheta_GE21;
//  std::unique_ptr<TH1F> GDPhi_GE11, GDPhi_GE21,GPPhi_GE11, GPPhi_GE21;
  std::unique_ptr<TH1F> diffTheta_GE11,diffTheta_GE21,diffPhi_GE11,diffPhi_GE21;
...

{
  ...
  HEPMCCol_Token    = consumes<edm::HepMCProduct>(edm::InputTag("generatorSmeared"));
  ...
  GDTheta_GE11 = std::unique_ptr<TH1F>(new TH1F("GDTheta_GE11","GDTheta_GE11",20,0,4));
  GDTheta_GE21 = std::unique_ptr<TH1F>(new TH1F("GDTheta_GE21","GDTheta_GE21",20,0,4));
  GPTheta_GE11 = std::unique_ptr<TH1F>(new TH1F("GPTheta_GE11","GPTheta_GE11",20,0,4));
  GPTheta_GE21 = std::unique_ptr<TH1F>(new TH1F("GPTheta_GE21","GPTheta_GE21",20,0,4));
  diffTheta_GE11 = std::unique_ptr<TH1F>(new TH1F("diffTheta_GE11","diffTheta_GE11",20,0,4));
  diffTheta_GE21 = std::unique_ptr<TH1F>(new TH1F("diffTheta_GE21","diffTheta_GE21",20,0,4));
  diffPhi_GE11 = std::unique_ptr<TH1F>(new TH1F("diffPhi_GE11","diffPhi_GE11",40,0,8));
  diffPhi_GE21 = std::unique_ptr<TH1F>(new TH1F("diffPhi_GE21","diffPhi_GE21",40,0,8));
...
TestGEMSegmentAnalyzer::~TestGEMSegmentAnalyzer()
{
  ...
  GPTheta_GE11->Write();
  GPTheta_GE21->Write(); 
  diffTheta_GE11->Write();
  diffTheta_GE21->Write();
  diffPhi_GE11->Write();
  diffPhi_GE21->Write();
...
TestGEMSegmentAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (auto gems = gemSegmentCollection->begin(); gems != gemSegmentCollection->end(); ++gems) {
    ...
    std::cout<<id.station()<<" "<<gemSegmentCollection->size()<<std::endl;
    ...
    std::cout<<"xx "<<segGD.theta()<<" "<<segGP.theta()<<std::endl;
    std::cout<<"xx "<<segGD.phi()<<" "<<segGP.phi()<<std::endl;
...
//      std::cout <<"   BX = "<<gems->bunchX()<<std::endl;
  ...
  GDTheta_GE11->Fill(segGD.theta());
  GPTheta_GE11->Fill(segGP.theta());
  diffTheta_GE11->Fill(fabs(segGD.theta()-segGP.theta()));
  diffPhi_GE11->Fill(fabs(segGD.phi()-segGP.phi()));
  //    GE11_BX->Fill(gems->bunchX());
      ...
      */
      ...
      if(id.region()>0) { GE11_Pos_XY->Fill(segGP.x(), segGP.y()); GEM_Pos_RZ->Fill(segGP.z(), segGP.perp()); ++nGEMrh[2];
       //      if(gems->bunchX()==0) {++nGEMrhbx0[2];}
       }
      if(id.region()<0) { GE11_Neg_XY->Fill(segGP.x(), segGP.y()); GEM_Neg_RZ->Fill(segGP.z(), segGP.perp()); ++nGEMrh[3]; 
       //      if(gems->bunchX()==0) {++nGEMrhbx0[3];}
       }
    }
	...
	GE21_CheckSegmentDirection_2hits->Fill((segGP.z()*segGD.z())/fabs(segGP.z()*segGD.z()));
      ...
      if(gems->nRecHits()>2) {
  GDTheta_GE21->Fill(segGD.theta());
  GPTheta_GE21->Fill(segGP.theta());
  diffTheta_GE21->Fill(fabs(segGD.theta()-segGP.theta()));
  diffPhi_GE21->Fill(fabs(segGD.phi()-segGP.phi()));
